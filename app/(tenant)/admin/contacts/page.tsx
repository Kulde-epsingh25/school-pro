...
export default function AdminContactsPage() {
  ...
  const fetchContacts = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NEXT_PUBLIC_API_URL || "https://school-pro-api-6mxq-5qzq.onrender.com")}/contacts${search ? `?search=${encodeURIComponent(search)}` : ""}`, { headers: { "x-user-id": user?.id || "" } });
      if (res.ok) {
