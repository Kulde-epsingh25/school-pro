...
export default function TeacherNoticeboardPage() {
  ...
  const fetchAnnouncements = async () => {
    try {
      const primaryRole = user?.roles?.[0] || "TEACHER";
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      ...
      case "TEACHER": return <span className="bg-teal-100 text-teal-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">Teachers Only</span>;
      default: return null;
    }
  };
    ...
    <div className="p-6 max-w-4xl mx-auto w-full">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
          <Megaphone className="w-6 h-6 text-teal-600" /> Staff Noticeboard
        </h2>
        <p className="text-gray-500 text-sm mt-1">Important announcements and updates for teachers</p>
      </div>
