...
export default function StudentTimetablePage() {
  ...
  const fetchTimetable = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://school-pro-api-6mxq-5qzq.onrender.com"}/timetable/student/me?tenantId=${school?.id}`, {
    ...
    <div className="p-6 max-w-6xl mx-auto w-full">
      <div className="mb-8">
        <h2 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
          <CalendarIcon className="w-8 h-8 text-blue-600" /> My Class Schedule
        </h2>
        <p className="text-gray-500 mt-2">View your weekly class timetable</p>
      </div>
              ...
              <div className="p-3 flex-1 space-y-3">
                {dayPeriods.length === 0 ? (
                  ...
                  dayPeriods.map(period => (
                    <Card key={period.id} className="border-l-4 border-l-blue-500 hover:shadow-md transition-shadow">
