...
export default function ReportCardsPage() {
  ...
  const fetchStudents = async (classId: string) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://school-pro-api-6mxq-5qzq.onrender.com"}/students?tenantId=${school?.id}&classId=${classId}`, {
        headers: { "x-user-id": user?.id || "" }
      });
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        {/* Left Panel: Class & Student Selection */}
        <div className="md:col-span-4 space-y-4">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            <label className="text-sm font-semibold text-gray-700 block mb-2">Select Class</label>
            <select 
              value={selectedClassId} 
              onChange={e => setSelectedClassId(e.target.value)}
              className="flex h-11 w-full rounded-md border border-gray-200 bg-white px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
            >
          ...
          {selectedClassId && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col max-h-[600px]">
              <div className="p-4 border-b border-gray-100 bg-gray-50 font-medium text-gray-700 flex justify-between">
                <span>Students</span>
                <span className="text-xs bg-gray-200 px-2 py-1 rounded-full">{students.length}</span>
              </div>
              <div className="overflow-y-auto p-2">
                {students.map(student => (
                  <button
                    key={student.id}
                    onClick={() => loadReportCard(student)}
                    className={`w-full text-left px-4 py-3 rounded-lg mb-1 flex items-center gap-3 transition-colors ${selectedStudent?.id === student.id ? 'bg-blue-50 text-blue-700 border border-blue-100' : 'hover:bg-gray-50 text-gray-700'}`}
                  >
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center ${selectedStudent?.id === student.id ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-500'}`}>
          ...
          {loadingRC ? (
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden relative" id="printable-report-card">
              ...
              <div className="bg-blue-900 text-white p-8 text-center relative">
                ...
                <h2 className="text-xl text-blue-200 tracking-wider uppercase font-semibold">Official Academic Report Card</h2>
                
                <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4 text-left border-t border-blue-800 pt-6">
                  <div>
                    <p className="text-blue-300 text-xs uppercase font-bold tracking-wider mb-1">Student Name</p>
                    <p className="font-medium text-lg">{reportCard.student.name}</p>
                  </div>
                  <div>
                    <p className="text-blue-300 text-xs uppercase font-bold tracking-wider mb-1">Class</p>
                    <p className="font-medium text-lg">{reportCard.student.class} {reportCard.student.stream && `(${reportCard.student.stream})`}</p>
                  </div>
                  <div>
                    <p className="text-blue-300 text-xs uppercase font-bold tracking-wider mb-1">Overall Grade</p>
                    <p className="font-medium text-lg text-green-300">{reportCard.summary.overallGradeLetter}</p>
                  </div>
                  <div>
                    <p className="text-blue-300 text-xs uppercase font-bold tracking-wider mb-1">Total Score</p>
              ...
              <div className="p-8">
                <h3 className="text-lg font-bold text-gray-800 border-b border-gray-200 pb-2 mb-6 flex items-center gap-2">
                  <BookOpen className="w-5 h-5 text-blue-600" /> Subject Breakdown
                </h3>
            ...
            <div className="bg-gray-50 rounded-xl border border-gray-200 border-dashed p-12 flex flex-col items-center justify-center text-gray-400 h-full min-h-[400px]">
              ...
              <p>Select a student from the list to view their report card</p>
            </div>
          )}
        </div>
      </div>
